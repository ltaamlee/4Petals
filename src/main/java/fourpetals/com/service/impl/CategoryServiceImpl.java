...
public class CategoryServiceImpl implements CategoryService {
    ...
    @Override
    public Optional<CategoryRowVM> get(Integer id) { return repo.findById(id).map(this::toVM); }
    ...
    public CategoryRowVM update(Integer id, CategoryRequest req) {
        ...
        if (req.tenDM()!=null) c.setTenDM(req.tenDM().trim());
        c.setMoTa(req.moTa());
        c = repo.save(c);
        return toVM(c);
    }
    ...
    public void delete(Integer id) {
        if (!repo.existsById(id)) throw new RuntimeException("Danh mục không tồn tại");
        repo.deleteById(id);
    }
}
