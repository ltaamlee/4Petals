...
let currentSort = { field: 'maDM', direction: 'asc' }; // mặc định sort theo maDM

const sortableColumns = [
	{ id: 'thMaDM', field: 'maDM' },
	{ id: 'thTenDM', field: 'tenDM' }
];
...
function loadCategories(page = 0) {
	...
	const url = `/api/manager/categories?page=${page}&size=${pageSize}&keyword=${encodeURIComponent(kw)}&sort=${sortParam}`;
	...
	fetch(url)
		...
		.then(data => {
			...
			updateSortIndicator(); // update mũi tên
		})
		.catch(() => {
			document.getElementById('catTableBody').innerHTML =
